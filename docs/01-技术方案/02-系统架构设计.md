...
├── defect_classifier.py  # 缺陷分类器（裂缝/蜂窝麻面）
└── utils/
    ├── tensorrt_engine.py # TensorRT引擎加载与推理
    ├── post_process.py   # 后处理（亚像素测量、骨架提取）
    └── calibrator.py     # 像素-毫米换算标定
```
...
src/gateway/
├── __init__.py
├── websocket_client.py   # WebSocket数据上报客户端
├── rtsp_client.py        # RTSP视频流拉取
├── ptz_controller.py     # 云台控制封装
