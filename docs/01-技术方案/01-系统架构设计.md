...
**硬件组成说明**：
绝影Lite3专业版为单机双主机架构，包含两个独立计算单元：
|- **运动主机（RK3588）**：负责运动控制、关节驱动、实时响应，运行绝影SDK
|- **感知主机（Jetson Xavier NX）**：负责视觉推理、算法执行、数据上报，运行TensorRT和AI模型

> **网络说明**：运动主机与感知主机共享同一IP地址 `192.168.1.103`，通过不同端口区分服务（UDP 43893 / WebSocket 8765 / RTSP 554）。此设计符合官方网络拓扑规范。

两个主机通过内部通信协作，对外通过网络接入项目局域网。
...
| 边缘层 | Jetson Xavier NX(192.168.1.103) | 视觉推理、算法执行、数据上报 | TensorRT + OpenCV |
...
> **注意**：原方案中设计了沙盘灯带联动功能（红色/黄色LED），但经核查机器狗硬件资料，绝影Lite3本体未提供用户可控的LED灯光接口，该功能暂无法实现。当前版本仅通过监测平台界面显示告警状态。
