...
本项目面向**2026年全国职业院校技能大赛**——机器狗电力巡检赛项，基于云深处科技绝影Lite3专业版四足机器人平台，实现抽水蓄能电站沙盘模型的自动化电力巡检演示。
...
| 成员 | 角色 | 主要职责 |
|------|------|----------|
| Wang Rongji | 环境工程师 | 现场测试环境搭建（地垫、踏板、台阶、模拟水泥面） |
| Li Zhangping | 硬件工程师 | 环境搭建、云台硬件集成、支架设计 |
| Chen Wei | 演示工程师 | 机器狗操作、演示执行、软件集成 |
| Chen Zili | 算法工程师 | 裂缝检测算法、温度监测算法、软件联调 |
...
本系统采用**感知主机 + 独立监测平台**的双部署架构，实现完整的电力巡检功能：
...
│  │  Server :8000│    │  Server :8765│    │  实时监控面板 │          │
│  └──────────────┘    └──────────────┘    └──────────────┘          │
│                           │                                         │
│                    ┌──────▼──────┐                                  │
│                    │  运动控制    │ ← W/↑/S/↓ A/D Q/E 键盘控制      │
│                    │  指令发送   │    Space=起立/趴下 ESC=急停      │
│                    └─────────────┘                                  │
└─────────────────────────────────────────────────────────────────────┘
                              │
...
│  ┌─────────────────────────────────────────────────────────┐       │
│  │                    DemoMain（主控制器）                   │       │
│  │  run_demo() · mode=simulation/real/hybrid               │       │
│  └───────────────┬───────────────┬─────────────────────────┘       │
│                  │               │                                   │
│  ┌───────────────▼──┐  ┌────────▼──────────┐                       │
│  │  UDPController   │  │  WebSocketGateway  │                       │
│  │  :43893/43894    │  │  → 监测平台        │                       │
│  └────────┬─────────┘  └───────────────────┘                       │
│           │                                                         │
│  ┌────────▼─────────┐  ┌──────────────────────────┐                │
│  │  RealBodyData    │  │  PtzController            │                │
│  │  (物理模型)       │  │  HTTP → 云台相机          │                │
│  └──────────────────┘  └──────────────────────────┘                │
│                                                                     │
│  ┌──────────────────┐  ┌──────────────────────────┐                │
│  │  Simulation      │  │  VideoStreamForwarder     │                │
│  │  Generator       │  │  RTSP → WebSocket         │                │
│  └──────────────────┘  └──────────────────────────┘                │
└─────────────────────────────────────────────────────────────────────┘
           │                                        │
           │ UDP :43893                            │ RTSP :554
           ▼                                        ▼
┌──────────────────────┐            ┌──────────────────────────┐
│  运动主机（RK3588）   │            │  云台相机 192.168.1.108   │
│  发送本体数据         │            │  可见光 + 热成像          │
└──────────────────────┘            └──────────────────────────┘
```
...
- **技术方案**: YOLOv8 + U-Net 深度学习模型
- **检测能力**: 自动识别 ≥0.1mm 混凝土裂缝
- **测量精度**: 宽度测量误差 <0.02mm
- **工作流程**: 
  1. 云台相机采集可见光图像
  2. AI模型推理检测裂缝
  3. 测量裂缝宽度和长度
  4. 通过WebSocket上报检测结果至监测平台
...
- **实时数据**: 
  ...
  - 超声波距离（前后障碍物）
- **上报频率**: 每5秒定时上报 system_status

#### 2.2.4 视频流转发

- **可见光流**: RTSP → WebSocket → 监测平台（15fps）
- **热成像流**: RTSP → WebSocket → 监测平台（15fps）
- **编码格式**: JPEG压缩

#### 2.2.5 运动控制

- **控制方式**: 键盘手柄（WASD/方向键 + Q/E + Space + ESC）
- **功能**:
  - W/↑: 前进
  - S/↓: 后退
  - A/←: 左移
