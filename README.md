...
| 项目 | 文档推荐 | 实际配置 | 影响 |
|------|----------|----------|------|
| Python版本 | 3.10+ | **3.8.10** | ✅ 兼容，可正常运行 |
| 内存配置 | 8GB+ | **6.7Gi** | ⚠️ 可用内存约5.4Gi，建议关闭其他应用 |
| 操作系统版本 | 22.04 LTS | **20.04.6 LTS** | ✅ 兼容 |

### 2.2 一键部署（推荐）

> **传输方式**: 请使用MobaXterm文件面板将 `lite3-power-inspection.zip` 复制粘贴至机器狗主机 `/home/ysc/` 目录
...
| 监测平台（FastAPI） | 192.168.1.103 | 8000 | HTTP | Web界面访问 |
| WebSocket网关 | 192.168.1.103 | 8765 | WebSocket | 实时数据推送 |

> **说明**：感知主机与运动主机共享同一IP地址 `192.168.1.103`，这是绝影Lite3官方设计的单机双主机架构。两个主机通过不同端口提供服务，互不冲突。
...
> **注意**：原设计包含沙盘灯带联动功能（温度预警时黄色灯带、告警时红色灯带），但经核查绝影Lite3硬件资料，机器狗本体未提供用户可控的LED灯光接口，该功能暂无法实现。当前版本仅通过监测平台界面显示告警状态。
...
| 编号 | 文档名称 | DOC编号 | 说明 |
|------|----------|---------|------|
| 01 | [系统架构设计](docs/01-技术方案/01-系统架构设计.md) | DOC-ARCH-001 | 模块划分、数据模型、时序图 |
| 02 | [API接口文档](docs/01-技术方案/02-API接口文档.md) | DOC-APISPEC-002 | WebSocket/HTTP/UDP接口定义 |
| 03 | [环境配置说明](docs/01-技术方案/03-环境配置说明.md) | DOC-ENV-003 | 环境配置快速参考 |
| 04 | [快速开始指南](docs/01-技术方案/04-快速开始指南.md) | DOC-START-004 | 首次运行与开发流程 |
| 05 | [项目代码规范](docs/01-技术方案/05-项目代码规范.md) | DOC-CODE-005 | 编码规范和Git流程 |
| 06 | [部署指南](docs/01-技术方案/06-部署指南.md) | DOC-DEPLOY-006 | 部署流程、启动测试、故障排查 |
| 07 | [数据采集规范](docs/01-技术方案/07-数据采集规范.md) | DOC-DATA-007 | 数据结构、命名规则、备份策略 |
| 08 | [环境诊断与故障排查指南](docs/01-技术方案/08-环境诊断与故障排查指南.md) | DOC-DIAG-008 | 一键诊断与故障排查 |
| 09 | [测试环境搭建指南](docs/01-技术方案/09-测试环境搭建指南.md) | DOC-TESTENV-009 | 测试环境准备 |
| 10 | [环境配置指南](docs/01-技术方案/10-环境配置指南.md) | DOC-CONFIG-010 | 主机配置、软件环境、网络拓扑 |
| 11 | [测试用例与验收标准](docs/01-技术方案/11-测试用例与验收标准.md) | DOC-TEST-011 | 功能测试、性能测试、验收标准 |

### 11.4 快速导航（推荐文档）

| 场景 | 推荐文档 | 说明 |
|------|----------|------|
| 🚀 **部署系统** | [06-部署指南](docs/01-技术方案/06-部署指南.md) | 一键部署、启动测试、故障排查 |
| 🔍 **环境诊断** | [08-环境诊断指南](docs/01-技术方案/08-环境诊断与故障排查指南.md) | 一键诊断脚本、问题排查 |
| ⚙️ **配置检查** | [10-环境配置指南](docs/01-技术方案/10-环境配置指南.md) | 主机配置、软件环境、网络拓扑 |
| 🧪 **测试验收** | [11-测试用例与验收标准](docs/01-技术方案/11-测试用例与验收标准.md) | 功能测试、性能测试、验收标准 |
| 📋 **环境速查** | [03-环境配置说明](docs/01-技术方案/03-环境配置说明.md) | 快速参考、一键命令 |
| 🛠️ **测试准备** | [09-测试环境搭建](docs/01-技术方案/09-测试环境搭建指南.md) | 测试环境要求、验证命令 |

### 11.3 项目管理文档

| 编号 | 文档名称 | 说明 |
|------|----------|------|
| 01 | [项目规划书](docs/02-项目管理/01-项目规划书.md) | 团队分工、物料清单、进度计划、风险管理 |
